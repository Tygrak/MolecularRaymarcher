...
fn dAtomsHardCoded(p: vec3<f32>) -> f32 {
    var atomNumber = -1.0;
    var resDistance = 1000000000.0;
    resDistance = opSMin(resDistance, dSphere(p, vec3(-5.538877, -14.736747, 14.020274), covalentRadius(5)), drawSettings.kSmoothminScale);
    
    /*
        generate using
        let hardcodedSdfString = "";
        for (let i = 0; i < atoms.length; i++) {
            const atom = atoms[i];
            hardcodedSdfString += "resDistance = opSMin(resDistance, dSphere(p, vec3("+atom.x.toFixed(6)+", "+atom.y.toFixed(6)+", "+atom.z.toFixed(6)+"), covalentRadius(5)), drawSettings.kSmoothminScale);\n";
        }
        console.log(hardcodedSdfString);
    */
...
fn fs_main(@builtin(position) position: vec4<f32>, @location(0) vPos: vec4<f32>) -> FragmentOutput {
	...
	for (iteration = 0; iteration < 50; iteration++) {
		if (t > 200.0) {
			return FragmentOutput(depthOutput, vec4(f32(iteration)/100.0, 0.0, 0.0, 1.0));
		}
        pos = start+t*rayDirection;
		let sdfResult = dAtomsAll(pos);
        
		if (sdfResult < 0.05) {
            return FragmentOutput(depthOutput, vec4(f32(iteration)/100.0, 1.0, 1.0, 0.0));
		}
		t = t+sdfResult;
	}
    return FragmentOutput(depthOutput, vec4(f32(iteration)/100.0, 0.0, 0.0, 1.0));
    */
    //#endif UseAllAtomsSDFRaymarch

    //this runs the hard coded stuff
    /*
    var t : f32 = 0.0;
    var pos : vec3<f32> = vec3(0.0);
    var iteration = 0;
	for (iteration = 0; iteration < 50; iteration++) {
        ...
        pos = start+t*rayDirection;
		let sdfResult = dAtomsHardCoded(pos);
